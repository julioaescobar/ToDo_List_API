...
def get_all_to_do(response: Response, 
    ...
    try:
        return todo_core.get_all_to_do(db,current_user.id)
...
def get_to_do(todo_id: int, 
              ...
              current_user = Depends(user_core.get_current_user)):
    try:
        todo_to_return = todo_core.get_to_do_by_id(db, todo_id=todo_id,current_user_id=current_user.id)
...
def create_to_do(todo: ToDo, 
                 ...
                 current_user = Depends(user_core.get_current_user)):
    try:
        return todo_core.create_to_do(db, todo=todo,current_user_id=current_user.id)
    except Exception as err:
        response.status_code = status.HTTP_500_INTERNAL_SERVER_ERROR
        print(err)
...
def update_to_do(todo: ToDo, 
                 ...
                 current_user = Depends(user_core.get_current_user)):
    try:
        if(todo_core.check_if_to_do_exist(db, todo_id,current_user.id)):
            return todo_core.update_to_do(db, todo_id, todo,current_user.id)
        response.status_code = status.HTTP_404_NOT_FOUND
    except Exception as err:
        response.status_code = status.HTTP_500_INTERNAL_SERVER_ERROR
        print(err)
...
def delete_to_do(todo_id: int, 
                 ...
                 current_user = Depends(user_core.get_current_user)):
    try:
        if(todo_core.check_if_to_do_exist(db, todo_id,current_user_id=current_user.id)):
            item_core.delete_items_by_todo_id(db,todo_id=todo_id,current_user_id=current_user.id)
            return todo_core.delete_to_do(db, todo_id,current_user.id)
        response.status_code = status.HTTP_404_NOT_FOUND
    except Exception as err:
        response.status_code = status.HTTP_500_INTERNAL_SERVER_ERROR
        print(err)
