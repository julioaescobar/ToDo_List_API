...
def get_all_items(todo_id: int, response: Response, db: Session = Depends(get_db)):
    try:
...
def get_items(todo_id: int, item_id: int, response: Response, db: Session = Depends(get_db)):
    try:
...
def create_item(todo_id: int, item_to_create: Item, response: Response, db: Session = Depends(get_db)):
    try:
...
def update_item(todo_id: int, item_id: int, response: Response, item_to_update: Item, db: Session = Depends(get_db)):
    try:
...
def delete_item(todo_id: int, item_id, response: Response, db: Session = Depends(get_db)):
    try:
        if(item_core.check_if_item_exist(db, todo_id, item_id)):
